        ...
        <div class="payroll-card">
            <div style="padding: 24px;">
                ...
                <p style="color: #718096; margin-bottom: 24px;">
                    {{ \Carbon\Carbon::parse($payrollPeriod->from_date)->format('M d, Y') }} -
                    {{ \Carbon\Carbon::parse($payrollPeriod->to_date)->format('M d, Y') }}
                </p>

                @if (empty($payrollResults))
                <div class="no-data">
                    <svg style="width: 64px; height: 64px; margin: 0 auto 16px; color: #cbd5e0;" fill="none"
                ...
                <div class="summary-card">
                    <div style="display: flex; align-items: center; margin-bottom: 16px;">
                        <svg style="width: 24px; height: 24px; margin-right: 12px;" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="summary-grid">
                        ...
                        <div class="summary-item">
                            ...
                            <div style="font-size: 24px; font-weight: 700;">
                                ₱{{ number_format(array_sum(array_column($payrollResults, 'gross_pay')), 2) }}</div>
                        </div>
                        <div class="summary-item">
                            <div style="font-size: 14px; opacity: 0.9;">Total Net Pay</div>
                            <div style="font-size: 24px; font-weight: 700;">
                                ₱{{ number_format(array_sum(array_column($payrollResults, 'net_pay')), 2) }}</div>
                        </div>
                    </div>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="amount-positive">
                                    ₱{{ number_format($result['gross_pay'] - $result['basic_hours_pay'], 2) }}</td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function openModal(employeeName, details) {
        ...
        let earningsList = ``;
        // Build earnings list dynamically to be more readable
        if (details.basic_hours_pay > 0) {
            earningsList +=
        ...
        if (details.overtime_pay > 0) {
            earningsList +=
                `<li><span>Regular OT Pay:</span> <span class="amount-positive">₱${details.overtime_pay.toFixed(2)}</span></li>`;
        }
        if (details.rest_day_pay > 0) {
            earningsList +=
                `<li><span>Rest Day Pay:</span> <span class="amount-positive">₱${details.rest_day_pay.toFixed(2)}</span></li>`;
        }
        if (details.rest_ot_pay > 0) {
            earningsList +=
                `<li><span>Rest Day OT Pay:</span> <span class="amount-positive">₱${details.rest_ot_pay.toFixed(2)}</span></li>`;
        }
        if (details.regular_holiday_pay > 0) {
            earningsList +=
                `<li><span>Regular Holiday Pay:</span> <span class="amount-positive">₱${details.regular_holiday_pay.toFixed(2)}</span></li>`;
        }
        if (details.special_holiday_pay > 0) {
            earningsList +=
                `<li><span>Special Holiday Pay:</span> <span class="amount-positive">₱${details.special_holiday_pay.toFixed(2)}</span></li>`;
        }
        if (details.rest_reg_pay > 0) {
            earningsList +=
                `<li><span>Reg. Holiday on Rest Day Pay:</span> <span class="amount-positive">₱${details.rest_reg_pay.toFixed(2)}</span></li>`;
        }
        if (details.rest_spec_pay > 0) {
            earningsList +=
                `<li><span>Spec. Holiday on Rest Day Pay:</span> <span class="amount-positive">₱${details.rest_spec_pay.toFixed(2)}</span></li>`;
        }
        if (details.ot_reg_holiday_pay > 0) {
            earningsList +=
                `<li><span>OT on Reg. Holiday:</span> <span class="amount-positive">₱${details.ot_reg_holiday_pay.toFixed(2)}</span></li>`;
        }
        if (details.ot_spec_holiday_pay > 0) {
            earningsList +=
                `<li><span>OT on Spec. Holiday:</span> <span class="amount-positive">₱${details.ot_spec_holiday_pay.toFixed(2)}</span></li>`;
        }
        if (details.night_differential_pay > 0) {
            earningsList +=
                `<li><span>Night Differential Pay:</span> <span class="amount-positive">₱${details.night_differential_pay.toFixed(2)}</span></li>`;
        }
        if (details.night_differential_pay_reg > 0) {
            earningsList +=
                `<li><span>Night Diff (Reg. Holiday):</span> <span class="amount-positive">₱${details.night_differential_pay_reg.toFixed(2)}</span></li>`;
        }
        if (details.night_differential_pay_spec > 0) {
            earningsList +=
                `<li><span>Night Diff (Spec. Holiday):</span> <span class="amount-positive">₱${details.night_differential_pay_spec.toFixed(2)}</span></li>`;
        }
        if (details.ot_night_diff_rdr_pay > 0) {
            earningsList +=
                `<li><span>Night Diff (OT on RDR):</span> <span class="amount-positive">₱${details.ot_night_diff_rdr_pay.toFixed(2)}</span></li>`;
        }
            ...
            <div class="detail-section">
                ...
                <div class="detail-list">
                    <ul style="list-style: none; margin: 0; padding: 0;">
                        ...
                        <li><span>Total Rest Day Hours:</span> <span>${details.total_rest_day_hours} hrs</span></li>
                        <li><span>Total Approved Overtime:</span> <span>${details.total_approved_overtime_hours} hrs</span></li>
                ...
                <div class="detail-list">
                    <ul style="list-style: none; margin: 0; padding: 0;">
                        ...
                        <li style="border-top: 2px solid #e2e8f0; margin-top: 8px; padding-top: 8px;">
                            ...
                            <span class="amount-total amount-positive">₱${(details.gross_pay).toFixed(2)}</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        document.body.style.overflow = 'hidden'; // Corrected to prevent scrolling
    }
    ...
    if (savePayrollBtn) {
        savePayrollBtn.addEventListener('click', function() {
            ...
            if (confirm('Are you sure you want to save this payroll? This action cannot be undone.')) {
                this.innerHTML =
                    '<i class="fas fa-spinner fa-spin" style="margin-right: 8px;"></i>Saving...';
                this.disabled = true;
                ...
                fetch('/payroll/save/{{ $payrollPeriod->id }}', {
                        ...
                        body: JSON.stringify({
                            payroll_results: payrollResultsData
                        })
                    })
                    .then(response => {
                        if (!response.ok) {
                            return response.text().then(text => {
                                throw new Error(
                                    `Server responded with status ${response.status}: ${text.substring(0, 200)}...`
                                );
                            });
                        }
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            this.innerHTML =
                                '<i class="fas fa-save" style="margin-right: 8px;"></i>Save Payroll';
                            this.disabled = false;
                        }
                    })
                    .catch(error => {
